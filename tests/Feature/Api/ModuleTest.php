...
class ModuleTest extends TestCase
{
    ...
    public function test_update_module_by_course()
    {
        ...
        $response = $this->putJson("/courses/{$course->uuid}/modules/{$module->uuid}", [
            'course' => $course->uuid,
            'name'   => 'Teste de Módulo'
        ]);

        $response->assertStatus(200);
    }

    public function test_notfound_module_by_course()
    {
        $course = Course::factory()->create();

        $response = $this->deleteJson("/courses/{$course->uuid}/modules/module_fake");
